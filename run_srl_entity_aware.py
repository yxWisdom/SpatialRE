...
from data_utils.entity_aware_srl_utils import EntityAwareSRLDataProcessor
from data_utils.srl_rule_utils import eval_all_link
from model.semantic_role_labeling import BertSRLEncoder, EntityAwareTransformerSoftmaxSRL, \
    EntityAwareTransformerCrfSRL
...
MODEL_CLASSES = {
    'entity_aware_transformer_softmax': (BertSRLEncoder, EntityAwareTransformerSoftmaxSRL, BertTokenizer),
    'entity_aware_transformer_crf': (BertSRLEncoder, EntityAwareTransformerCrfSRL, BertTokenizer)
}
...
def evaluate(args, model, tokenizer, processor, prefix="", save_predict=True):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        result = processor.evaluate(predicts, tokenizer)
        full_result = processor.evaluate_link(predicts, tokenizer, metric=Metrics.STRICT, eval_optional_roles=True)
        part_result = processor.evaluate_link(predicts, tokenizer, metric=Metrics.STRICT, eval_optional_roles=False)

        all_result ={
            "result": result,
            "strict_full": full_result,
            "strict_part": part_result,
        }
...
def load_and_cache_examples(args, processor, tokenizer, evaluate=False):
    ...
    all_features = (all_input_ids, all_input_mask, all_segment_ids, all_label_ids)
...
def main():
    ...
    parser.add_argument("--model_name_or_path", default=None, type=str, required=True,
                        help="Path to pre-trained model or shortcut name selected in the list: " + ", ".join(
                            ALL_MODELS))
    parser.add_argument("--task_name", default="SRL", type=str, required=False,
    ...
    processor = EntityAwareSRLDataProcessor(data_dir=args.data_dir,
