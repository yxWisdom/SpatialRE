...
from model.mhs import Config1Model1, Config1Model2
...
MODEL_CLASSES = {
    'ea_transformer_config1_1': (BertEncoder, Config1Model1, BertTokenizer, SpaceEvalConfig1DataProcessor_1),
    'ea_transformer_config1_2': (BertEncoder, Config1Model2, BertTokenizer, SpaceEvalConfig1DataProcessor_2)
}
...
def evaluate(args, model, tokenizer, processor, prefix="", dataset=None, save_predict=True):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        element_result = processor.evaluate_element(pred_result, eval_full=False)
        element_full_result = processor.evaluate_element(pred_result, eval_full=True)
        attr_result = processor.evaluate_link_attribute(pred_result)
        link_result = processor.evaluate_exact(pred_result, Metrics.STRICT, eval_link_attr=False)
        strict_link_result = processor.evaluate_exact(pred_result, Metrics.STRICT, eval_link_attr=True)

        all_result = {
            "part element": element_result,
            "full element": element_full_result,
            "attribute": attr_result,
            "link": link_result,
            "strict link": strict_link_result
        }

        results.update(strict_link_result["OVERALL"])
...
def main():
    ...
    if args.model_type == "ea_transformer_config1_2":
