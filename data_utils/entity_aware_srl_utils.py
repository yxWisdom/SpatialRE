...
class InputExample(object):
    ...
    def __init__(self, guid, text, file=None, tokens=None, labels=None, element_ids=None, features=None):
        if features is None:
            features = {}
        self.guid = guid
        self.file = file
...
class EntityAwareSRLDataProcessor:
    ...
    def _read_labels(self):
        ...
        if self.use_x_tag:
            label_set.add("X")
        self.labels = list(sorted(sorted(label_set, key=lambda x: x[:1]), key=lambda x: x[1:]))
    ...
    def _create_example(self, lines, set_type):
        ...
        for i, line in enumerate(lines):
            ...
            filename = trigger_info[3]
            tokens = columns[1].split()
            labels = columns[-1].split()
            element_ids = list(map(lambda x: x if x != "O" else "", columns[-2].split()))
            ...
            text = " ".join(tokens)
            examples.append(
                InputExample(guid=guid, file=filename, text=text, tokens=tokens, labels=labels, element_ids=element_ids,
                             features=features))
        return examples
    ...
    def evaluate(self, predict_label_ids, tokenizer, set_type="dev"):
        ...
        total_num, accuracy, total_correct_num, total_predict_num, total_gold_num, precision, recall, f_1 = eval_srl(
            lines)
        eval_dict[
    ...
    def get_links(self, predict_label_ids, tokenizer, set_type="dev"):
        examples = self.dev_examples if set_type == 'dev' else self.test_examples
        ...
        for example, p_label_ids in zip(examples, predict_label_ids):
            ...
            for token in example.tokens:
                ...
                new_p_label_ids.append(p_label_ids[start_idx])
                start_idx += len(sub_tokens)

            p_labels = [self.labels[id_] for id_ in new_p_label_ids]
            link_type = None
            type2roles = defaultdict(set)
            for p_label, element_id in zip(p_labels, example.element_ids):
                if p_label.startswith("B-"):
                    p_label = p_label[2:]
                    if p_label.endswith("trigger"):
                        link_type = "MOVELINK" if p_label[0] == "M" else "OLINK" if p_label[0] == "O" else "QSLINK"
                        p_label = "trigger"
                    type2roles[p_label].add(element_id)

            if link_type is None:
                continue

            role_group = ()
            for role_type in self.link2role_type[link_type]:
                role_set = type2roles.get(role_type, {""})
                if role_type == "motion_signalID":
                    roles_str = ",".join(sorted(map(str, role_set)))
                    role_group += ([roles_str],)
                else:
                    role_group += (role_set,)

            for tuple_ in itertools.product(*role_group):
                file2links[filename][link_type].add((link_type,) + tuple_)
    ...
    def evaluate_link(self, predict_label_ids, tokenizer, metric: Metrics, eval_optional_roles=True, set_type="dev"):
        ...
        file2links = self.get_links(predict_label_ids, tokenizer, set_type)

        if eval_optional_roles and "B-source" not in self.labels:
            eval_optional_roles = False

        for file, pred_link_dict in file2links.items():
            path = os.path.join(self.data_dir, 'xml', file)
            gold_link_dict = load_links_from_file(path, metric, eval_optional_roles)

            for link_type in self.link2role_type.keys():
