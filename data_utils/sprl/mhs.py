...
class SpRLBaseMHSDataProcessor(EntityAwareMHSDataProcessor):
    ...
    def evaluate_link(self, pred_triples_list, task: SpRLTask = SpRLTask.TASK_1, strict=True):
        ...
        for i, (example, ori_pred_triples, gold_link_dict) in enumerate(
                zip(self.dev_examples, pred_triples_list, gold_result)):
    ...
    def save_xml(self, output_dir, pred_result):
        ...
        root = tree.getroot()
        for sentence, triples in zip(root.findall("SENTENCE"), pred_result["links"]):
            sentence_text = ""
            ...
            sen_len = len(tokens)
            pred_triples = self.decode_triples(tokens, triples)
