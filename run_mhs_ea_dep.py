...
from data_utils.entity_aware_dep_mhs_utils import EntityAwareMHSDataProcessor_2
from data_utils.mhs_utils import processors
from model.mhs import EATransformerMHS, DepEATransformerMHS
...
MODEL_CLASSES = {
    ...
    'dep_entity_aware_transformer': (BertSRLEncoder, DepEATransformerMHS, BertTokenizer)
...
def train(args, train_dataset, model, tokenizer, processor, eval_dataset=None):
    ...
    model.zero_grad()
    # train_iterator = trange(int(args.num_train_epochs), desc="Epoch", disable=args.local_rank not in [-1, 0], mininterval=10)
...
def evaluate(args, model, tokenizer, processor, prefix="", dataset=None, save_predict=True):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        strict_result = processor.evaluate_1(pred_triples, Metrics.STRICT)
...
def load_and_cache_examples(args, processor, tokenizer, evaluate=False):
    ...
    if args.use_dependency:
        all_features += (torch.tensor([f.dependency_graph for f in features], dtype=torch.long),)
...
def main():
    ...
    processor = EntityAwareMHSDataProcessor_2(data_dir=args.data_dir,
                                              used_feature_names=config_dict["used_feature_name"],
                                              tokenizer=tokenizer, use_head=args.use_head)
    ...
    # args.use_dependency = config_dict["use_rel_pos"]
    args.use_dependency = True
