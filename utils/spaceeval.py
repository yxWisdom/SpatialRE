    ...
    with open(filepath, "r", encoding="utf-8") as file:
        for _, groups in groupby(file, lambda x: x.strip() == ""):
            ...
            for link in groups[1:]:
                ...
                type2links[link_type].add(f"{filename}\t{link}")
    return type2links
...
def evaluate(gold_path, pred_path):
    ...
    for link_type in gold_result.keys():
        gold_links = gold_result[link_type]
        pred_links = pred_result[link_type]
