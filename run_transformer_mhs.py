...
from data_utils.mhs_utils import convert_examples_to_features, processors
from model.mhs import MultiHeadSelection
from model.mhs_transformer import TransformerMHS
from model.multi_head_selection import BertMultiHeadSelection, TransformerMultiHeadSelection
...
MODEL_CLASSES = {
    'transformer': (BertConfig, TransformerMHS, BertTokenizer),
...
def evaluate(args, model, tokenizer, processor, prefix="", dataset=None, save_predict=True):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        result = processor.evaluation(pred_triples)
        results.update(result["All"])
...
def load_and_cache_examples(args, processor, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        relation_list = processor.get_relations()
        examples = processor.get_dev_examples(args.data_dir) if evaluate else processor.get_train_examples(
            args.data_dir)
        features = convert_examples_to_features(examples, label_list, relation_list, args.max_seq_length, tokenizer,
    ...
    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_selection_matrix)
    return dataset
...
def main():
    ...
    processor = processors[args.task_name](data_dir=args.data_dir, tokenizer=tokenizer, use_head=args.use_head)

    mhs_params = json.load(open(args.config_dir))
    # mhs_params = config_class.from_pretrained(args.config_dir)
    mhs_params.update({"labels": processor.get_labels(), "relations": processor.get_relations()})
    config.__dict__.update({"mhs_params": mhs_params})

    # setattr(config, 'labels', processor.get_labels())
    # setattr(config, 'relations', processor.get_relations())
