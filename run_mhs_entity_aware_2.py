            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:
                        results, pred_triples = evaluate(args, model, tokenizer, processor, prefix=global_step,
                                                         dataset=eval_dataset, save_predict=False)
                        ...
                        for k, v in metrics.items():
                            f1 = best_f1_dict.get(k, v['f1'])
                            if v['f1'] >= f1:
                                print(f"{k} current best f1: {v['f1']}, epoch: {epoch}")
                                best_f1_dict[k] = v['f1']
                                processor.save_predict_result(args.output_dir, pred_triples, suffix=k)
...
def evaluate(args, model, tokenizer, processor, prefix="", dataset=None, save_predict=True):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if save_predict:
            processor.save_predict_result(eval_output_dir, pred_triples, suffix=prefix)
        ...
        results['metrics'] = full_result

        overall = {}
        for key in full_result["OVERALL"]:
            overall[key] = (full_result["OVERALL"][key] + part_result["OVERALL"][key]) / 2
...
def load_and_cache_examples(args, processor, tokenizer, evaluate=False):
    ...
    matrix_dtype = torch.float32 if len(features[0].selection_matrix.shape) == 3 else torch.long
    all_selection_matrix = torch.tensor([f.selection_matrix for f in features], dtype=matrix_dtype)
...
def main():
    ...
    processor_class = DATA_PROCESSORS[args.model_type]
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, _ = evaluate(args, model, tokenizer, processor, prefix=global_step, save_predict=True)
