...
from data_utils.srl_rule_utils import processors, convert_examples_to_features
from model.bert_srl_rule import BertTransformerCrfWithRuleSRL
...
MODEL_CLASSES = {
    ...
    "transformer_crf": (BertConfig, BertTransformerCrfWithRuleSRL, BertTokenizer),
}
...
def load_and_cache_examples(args, processor, tokenizer, evaluate=False):
    ...
    all_pretag_ids = torch.tensor([f.pretag_ids for f in features], dtype=torch.long)

    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_predicate_mask,
                            all_tag_ids, all_pretag_ids)
    return dataset
