...
from data_utils.sprl.mhs_triple import SpRLMHSDataProcessTask1, SpRLMHSDataProcessTask2, SpRLMHSDataProcessTask1_softmax, SpRLMHSDataProcessTask2_softmax
from model.mhs import EATransformerMHS, EATransformerMHSSoftmax
from model.sprl.mhs_triple import SpRLModel_B, SpRLModel_B_softmax, JointSpRLModel_B, JointSpRLModel_B_softmax
...
MODEL_CLASSES = {
    "task_1_pipeline": (BertEncoder, EATransformerMHS, BertTokenizer, SpRLMHSDataProcessTask1),
    "task_2_pipeline": (BertEncoder, SpRLModel_B, BertTokenizer, SpRLMHSDataProcessTask2),

    "task_1_pipeline_sfm": (BertEncoder, EATransformerMHSSoftmax, BertTokenizer, SpRLMHSDataProcessTask1_softmax),
    "task_2_pipeline_sfm": (BertEncoder, SpRLModel_B_softmax, BertTokenizer, SpRLMHSDataProcessTask2_softmax),

    "task_1_joint": (BertEncoder, JointSpRLModel_B, BertTokenizer, SpRLMHSDataProcessTask1),
    "task_2_joint": (BertEncoder, JointSpRLModel_B, BertTokenizer, SpRLMHSDataProcessTask2),

    "task_1_joint_sfm": (BertEncoder, JointSpRLModel_B_softmax, BertTokenizer, SpRLMHSDataProcessTask1_softmax),
    "task_2_joint_sfm": (BertEncoder, JointSpRLModel_B_softmax, BertTokenizer, SpRLMHSDataProcessTask2_softmax),
}
...
def train(args, train_dataset, model, tokenizer, processor, eval_dataset=None):
    ...
    train_relations = processor.get_train_data()
    train_element_ids = processor.get_element_ids("train")
    #######
    ...
    for epoch in range(int(args.num_train_epochs)):
        ...
        for step, batch in enumerate(epoch_iterator):
            ...
            inputs = {'input_ids': batch[0],
                      ...
                      "token_to_pre_idx": batch[-1]
                      }
            if args.use_rel_pos:
                inputs['relative_positions'] = batch[4 + features_num]
                inputs['entity_mask'] = batch[5 + features_num]

            # if not args.model_type.startswith("task_1"):
            #     inputs["gold_rel_type_dict"] = {link_type: batch[i + 6 + features_num] for i, link_type in enumerate(processor.link_types)}

            if not args.model_type.__contains__("pipeline"):
                inputs['gold_element_labels'] = batch[4]

            inputs["gold_relations"] = train_relations[cursor: cursor+args.train_batch_size]
            inputs["element_ids"] = train_element_ids[cursor: cursor+args.train_batch_size]
...
def evaluate(args, model, tokenizer, processor, prefix="", dataset=None, save_predict=True):
    ...
    features_num = len(processor.get_features_size())
    test_element_ids = processor.get_element_ids("dev")
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        for batch in tqdm(eval_dataloader, desc="Evaluating", mininterval=5):
            ...
            with torch.no_grad():
                ...
                inputs["element_ids"] = test_element_ids[cursor: cursor + args.eval_batch_size]
                cursor += args.eval_batch_size
                ...
                pred_result.extend(batch_result)

            nb_eval_steps += 1

        eval_loss = eval_loss / nb_eval_steps

        pred_result = processor.post_process(pred_result)
        if save_predict:
            processor.save_predict_result(eval_output_dir, pred_result)
        all_result = processor.evaluate(pred_result)

        if args.model_type.startswith("task_1"):
            results.update(all_result["task1"]["OVERALL"])
        elif args.model_type.startswith("task_2"):
            results.update(all_result["task2"]["OVERALL"])
        else:
            results.update(all_result["task3"]["OVERALL"])
...
def load_and_cache_examples(args, processor, tokenizer, evaluate=False):
    ...
    if features[0].token_to_pre_idx is not None:
        all_features += (torch.tensor([f.token_to_pre_idx for f in features], dtype=torch.long),)
...
def main():
    ...
    parser.add_argument("--max_distance", default=0, type=int, required=False)
    ...
    config_dict["model_params"].update({"labels": processor.get_labels()})

    if args.model_type != "task_1_pipeline":
        label_dict = {
            "element_label": processor.get_element_labels(),
            "rel_type_label": processor.link2rel_types
        }
        config_dict["model_params"].update({"label_dict": label_dict})

    args.use_rel_pos = config_dict["use_rel_pos"]
    if args.max_distance == 0:
        args.max_distance = config_dict["model_params"]["max_distance"]
    else:
        config_dict["model_params"]["max_distance"] = args.max_distance
