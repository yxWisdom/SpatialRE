...
from data_utils.srl_utils import processors, convert_examples_to_features
from model.bert_srl import BertBasicSRL, BertCrfSRL, BertLSTMSRL, BertLSTMCrfSRL, BertTransformerSRL, \
    BertTransformerCrfSRL
...
MODEL_CLASSES = {
    'bert': (BertConfig, BertBasicSRL, BertTokenizer),
    'bert_crf': (BertConfig, BertCrfSRL, BertTokenizer),
    'bert_lstm': (BertConfig, BertLSTMSRL, BertTokenizer),
    'bert_lstm_crf': (BertConfig, BertLSTMCrfSRL, BertTokenizer),
    "transformer": (BertConfig, BertTransformerSRL, BertTokenizer),
    "transformer_crf": (BertConfig, BertTransformerCrfSRL, BertTokenizer),
                ...
                if args.local_rank in [-1, 0] and args.logging_steps > 0 and global_step % args.logging_steps == 0:
                    ...
                    if args.local_rank == -1 and args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        ...
                        if results['f-1'] > best_f1:
                            ...
                            shutil.copy(os.path.join(args.output_dir, "predict.txt"), output_dir)
                            shutil.copy(os.path.join(args.output_dir, "predict_srl_err.txt"), output_dir)
...
def evaluate(args, model, tokenizer, processor, prefix="", save_predict=True):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if preds.shape != out_label_ids.shape:
            preds = np.argmax(preds, axis=-1)
        active_index = out_label_ids >= 0
        active_preds = preds[active_index]
        active_labels = out_label_ids[active_index]
        atten_labels = [i for i, label in enumerate(processor.get_labels()) if label not in ["O", "X"]]
        result = acc_and_f1(active_preds, active_labels, attention_labels=atten_labels)
        results.update(result)

        eval_dict = processor.evaluate(preds, tokenizer, set_type="dev")
        result.update(eval_dict)
        results['accuracy'] = result['accuracy']
        results["f-1"] = result['f-1']
        if save_predict:
            processor.save_predict(eval_output_dir, preds, tokenizer, set_type="dev")
...
def load_and_cache_examples(args, processor, tokenizer, evaluate=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        features = convert_examples_to_features(examples, label_list, tag_list, args.max_seq_length, tokenizer,
    ...
    dataset = TensorDataset(all_input_ids, all_input_mask, all_segment_ids, all_label_ids, all_predicate_mask,
                            all_tag_ids)
    return dataset
...
def main():
    ...
    if args.model_type.endswith("_crf"):
        model = model_class.from_pretrained(args.model_name_or_path, from_tf=bool('.ckpt' in args.model_name_or_path),
                                            config=config, labels=label_list, encoding_type='BIO')
    else:
        model = model_class.from_pretrained(args.model_name_or_path, from_tf=bool('.ckpt' in args.model_name_or_path),
                                            config=config)
